...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
function collectBannerAssets(bannerPaths: any): Array<{ url: string; filename: string }> {
  const out: Array<{ url: string; filename: string }> = []
  if (!bannerPaths) return out
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  Object.values(bannerPaths).forEach((b: any) => {
...
export function buildPublishAssetsScript(article: Article): string {
