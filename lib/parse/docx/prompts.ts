import type { ExhibitPaths } from '@/types'
...
interface ExtractionPromptParams {
  ...
  hyperlinksContext: string
}

export function buildExhibitsContext(exhibitsSummary: ExhibitPaths | null | undefined): string {
...
export function buildHyperlinksContext(hyperlinks: Hyperlink[]): string {
...
export function buildExtractionPrompt({
  ...
  exhibitsContext,
  hyperlinksContext,
}: ExtractionPromptParams): string {
  return `Extract structured data from this MSCI Research intake document. Return ONLY valid JSON, no markdown, no backticks.

CRITICAL JSON RULES:
- Properly escape all strings. No raw newlines inside JSON strings.
- Inside any HTML string value, NEVER use straight double quotes (") for editorial quotes around words or phrases. Use curly quotes ("…") instead. Example: write <p>such as "retirement," "vacation"</p>, NOT <p>such as "retirement," "vacation"</p>.
- HTML attribute delimiters (e.g. href="...") are the only places straight double quotes may appear inside HTML strings, and those MUST be escaped as \\".
...
export function buildTagsPrompt(tagsText: string): string {
