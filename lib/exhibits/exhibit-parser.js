 ...
 * Exhibit parser
 *
 * Parses SharePoint exhibit filenames into structured items. All exhibits
 * (static + interactive) live in the same Charts/Exhibits folder and carry
 * a numeric order suffix at the end of their filename:
 *
 ...
 *
 * The order number is authoritative and matches the order of appearance
 * in the blog post (1 = first exhibit in the document, 2 = second, …).
 *
 * Exports:
 *   parseExhibitFilename — single filename → {order, kind, baseName, ext}
 *   buildExhibitItems    — files[] → items[] (authoritative model)
 *   buildExhibitPaths    — files[] → { items, statics, interactives, summary }
 *                          where statics/interactives/summary are kept for
 *                          backward compatibility with the chrome extension
 *                          and existing UI dropdowns.
 */
