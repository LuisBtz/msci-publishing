/**
 * POST /api/parse/docx/reprocess
 *
 * Re-fetches the Word document from SharePoint, re-parses it through
 * the same Claude pipeline, and returns the new fields alongside a
 * `changes` map that flags which top-level fields differ from the
 * current article. The client uses `changes` to highlight updated text
 * in green.
 *
 * Detection strategy:
 *   1. Extract raw text from the NEW docx via mammoth
 *   2. Re-run the Claude extraction pipeline
 *   3. Compare the NEW structured output field-by-field against the
 *      CURRENT article stored in Supabase
 *   4. Also compare the raw text hash — if raw text changed,
 *      guarantee at least body_blocks are flagged as changed
 *
 * Expects JSON body: { articleId: string }
 */
...
/** Strip all HTML tags + collapse whitespace for plain-text comparison. */
...
/** Light normalisation — collapse whitespace only. */
function norm(s) {
  return (s || '').replace(/\s+/g, ' ').trim()
}

/** Compare two scalars after normalisation. */
function diffScalar(a, b) {
  return norm(String(a ?? '')) !== norm(String(b ?? ''))
}

/** Compare two arrays of strings (bullets). Per-index changed flags. */
function diffStringArray(oldArr, newArr) {
...
/** Compare two body_blocks arrays. Per-index changed flags. */
function diffBodyBlocks(oldBlocks, newBlocks) {
...
/** Compare two footnote arrays. Per-index changed flags. */
function diffFootnotes(oldFn, newFn) {
...
export async function POST(req) {
  try {
    ...
    try {
      ...
      console.error('[reprocess] JSON parse failed:', message1.content[0].text.substring(0, 500))
