...
export function populateStep3() {
  ...
  ;(document.getElementById('btn-create-page') as HTMLButtonElement).disabled = false
...
export function renderStep3Validation(article) {
  ...
  if (pageCheck?.exists) {
    ...
    ;(document.getElementById('link-edit-page') as HTMLAnchorElement).href = editUrl
    ;(document.getElementById('link-sites-view') as HTMLAnchorElement).href = sitesUrl
...
export function buildCreatePageParams(article) {
  ...
  const allBanners = Object.values(article.banner_paths || {}) as any[]
