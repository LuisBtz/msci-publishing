...
async function phaseContent(errors) {
    ...
    if (bullets.length > 0) {
        ...
        if (kfResult.success)
            modules.push('keyFindings');
        else
            errors.push('Key Findings injection failed');
        renderGlobalLog();
    }
    // 2. Body Content
    ...
    if (bodyBlocks.length > 0) {
        ...
        if (bodyResult.success)
            modules.push('bodyContent');
        else
            errors.push('Body Content injection failed');
        renderGlobalLog();
    }
    // 3. Authors
    ...
    if (authorPaths.length > 0) {
        ...
        if (authResult.success)
            modules.push('authors');
        else
            errors.push('Authors injection failed');
        renderGlobalLog();
    }
    // 4. Related Content
    ...
    if (relatedItems.length > 0) {
        ...
        if (relResult.success)
            modules.push('relatedContent');
        else
            errors.push('Related Content injection failed');
        renderGlobalLog();
    }
    // 5. Footnotes
    ...
    if (footnotes.length > 0) {
        ...
        if (fnResult.success)
            modules.push('footnotes');
        else
            errors.push('Footnotes injection failed');
        renderGlobalLog();
    }
...
function buildAuthorPaths(article) {
    ...
    return (article.authors || [])
        .map((a) => {
        if (a.content_fragment_path)
            return a.content_fragment_path;
...
function buildRelatedItems(article) {
    ...
    function toAemPath(url) {
        ...
        try {
            ...
            if (!host.endsWith('msci.com'))
                return url;
            if (host === 'support.msci.com')
                return url;
            if (path.startsWith('/indexes/index/'))
                return url;
            if (path.startsWith('/indexes/'))
                return IPC_ROOT.replace('/indexes', '') + path;
...
function setPhaseRunning(card, text) {
    ...
    if (status)
        status.innerHTML = `<span class="phase-spinner"></span> ${escHtml(text)}`;
}
function setPhaseComplete(card, text) {
    ...
    const status = card.querySelector('.phase-status');
    if (status)
        status.innerHTML = `<span class="phase-check">&#10003;</span> ${escHtml(text)}`;
}
function setPhaseError(card, text) {
    ...
    const status = card.querySelector('.phase-status');
    if (status)
        status.innerHTML = `<span class="phase-x">&#10007;</span> ${escHtml(text)}`;
}
function setPhaseWarning(card, text) {
    ...
    const status = card.querySelector('.phase-status');
    if (status)
        status.innerHTML = `<span class="phase-warn">!</span> ${escHtml(text)}`;
}
