...
export function populateStep2() {
  ...
  const startBtn = document.getElementById('btn-start-process') as HTMLButtonElement
...
export async function startProcess() {
  const btn = document.getElementById('btn-start-process') as HTMLButtonElement
  btn.disabled = true
  ...
  if (pageCheck?.exists) {
    ...
    const userChoice = await new Promise((resolve) => {
      ...
      document.getElementById('btn-save-custom-slug').addEventListener('click', async () => {
        const newSlug = (document.getElementById('input-custom-slug') as HTMLInputElement).value.trim()
        if (!newSlug) return
