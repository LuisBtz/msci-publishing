...
function paintEnvToggle(env: string) {
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  document.querySelectorAll('#aem-env-toggle .env-btn').forEach((btn) => {
    btn.addEventListener('click', async () => {
      const next = (btn as HTMLElement).dataset.env
  ...
  document.querySelectorAll('.chip').forEach((chip) => {
    chip.addEventListener('click', () => {
      ...
      state.currentFilter = (chip as HTMLElement).dataset.filter
      renderArticles()
    })
  })
  ...
  document.querySelectorAll('#steps-bar .step').forEach((el) => {
    el.addEventListener('click', () => handleStepIndicatorClick(el as HTMLElement))
  })
