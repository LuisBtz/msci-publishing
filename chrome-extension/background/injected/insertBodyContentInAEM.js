...
export async function insertBodyContentInAEM(slug, bodyBlocks) {
    ...
    async function createStaticExhibitBlock(gridPath, block, damBase, token, log, logErr) {
        ...
        if (block.title) {
            ...
            if (res.ok)
                ...
                logErr('  Error título: HTTP ' + res.status);
        }
        // Image component
        if (block.desktopFilename) {
            ...
            if (res.ok)
                log('  Imagen configurada ✓ → ' + block.desktopFilename);
            else
                logErr('  Error imagen: HTTP ' + res.status);
        }
        // Caption RTE
        if (block.caption) {
            ...
            if (res.ok)
                ...
                logErr('  Error caption: HTTP ' + res.status);
        }
    }
    ...
    async function createInteractiveExhibitBlock(gridPath, block, damBase, token, log, logErr) {
        ...
        if (block.jsonFilename) {
            ...
            if (res.ok)
                log('  Vega chart configurado ✓ → ' + block.jsonFilename);
            else
                logErr('  Error vega chart: HTTP ' + res.status);
        }
        // Caption RTE
        if (block.caption) {
            const capPath = gridPath + '/richtexteditor_caption';
    ...
    function buildBodyRtePayload(rawHtml) {
        ...
        html = html.replace(/<h2>([\s\S]*?)<\/h2>/gi, (_, inner) => `<h2><span class='${H2_CLASS}'>${inner}</span></h2>`);
        html = html.replace(/<h3>([\s\S]*?)<\/h3>/gi, (_, inner) => `<h3><span class='${H3_CLASS}'>${inner}</span></h3>`);
        // Wrap paragraph content in styled span (skip already-wrapped ones)
        html = html.replace(/<p>([\s\S]*?)<\/p>/gi, (match, inner) => {
            if (inner.includes(H2_CLASS) || inner.includes(H3_CLASS))
                return match;
    ...
    function walkInlineChildren(el) {
        ...
        for (const node of el.childNodes) {
            if (node.nodeType === 3) {
                ...
                if (text)
                    result.push({ tag: 'text', textContent: text });
            }
