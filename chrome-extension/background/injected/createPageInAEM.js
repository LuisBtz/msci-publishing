  ...
  try {
    ...
    try {
      ...
      if (!rt) {
        ...
        const rtProp =
          (siblingProps && rtCandidates.find((c) => c in siblingProps && Number(siblingProps[c]) > 0)) || 'time'
    ...
    try {
      if (!authorPaths || !authorPaths.length) {
        ...
        if (siblingAuthors) {
          ...
          if (items.length) {
            const props = Object.keys(siblingAuthors[items[0]]).filter(
              (k) => !k.startsWith('jcr:') && !k.startsWith('sling:') && !k.startsWith(':')
            )
    ...
    try {
      if (!thumbnailDamPath) {
        ...
        if (siblingProps) {
          const extraNodes = Object.entries(siblingProps).filter(
            ([k, v]) =>
              typeof v === 'object' &&
              v !== null &&
              typeof v.fileReference === 'string' &&
              v.fileReference.includes('/content/dam/') &&
              k !== 'image' &&
