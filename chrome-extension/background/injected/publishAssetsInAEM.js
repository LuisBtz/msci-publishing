    ...
    try {
        ...
        async function uploadAsset(sourceUrl, damFolderPath, filename) {
            try {
                ...
                if (!srcRes.ok)
                    return { ok: false, detail: 'source fetch failed: HTTP ' + srcRes.status };
                const buffer = await srcRes.arrayBuffer();
                if (!buffer.byteLength)
                    return { ok: false, detail: 'empty response from source URL' };
                ...
                if (!putRes.ok)
                    return { ok: false, detail: 'blob PUT failed: HTTP ' + putRes.status };
        ...
        function logResult(filename, res) {
            if (res.ok)
                log(`   ✅ ${filename} (${Math.round(res.size / 1024)}KB, ${res.mime})`);
            else
                logErr(`   ❌ ${filename} — ${res.detail || 'HTTP ' + res.status}`);
        }
        let s;
        ...
        if (exhibitAssets.length === 0)
            logWarn('   No exhibits to upload');
