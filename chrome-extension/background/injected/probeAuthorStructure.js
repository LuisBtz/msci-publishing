...
export async function probeAuthorStructure(slug) {
    ...
    try {
        ...
        for (const sib of siblings) {
            if (bestAuthorCount >= 4)
                break; // found a great example
            let tree;
            try {
            ...
            function walk(node, path, ancestors) {
                ...
                if (resType === CONTAINER_TYPE) {
                    allContainers.push({
                        ...
                        childKeys: Object.keys(node).filter((k) => !k.startsWith('jcr:') && !k.startsWith('sling:') && !k.startsWith('cq:') && !k.startsWith(':') && typeof node[k] === 'object'),
                    });
                }
            ...
            for (const cf of cfNodes) {
                ...
                const pathLike = propKeys.filter((k) => typeof cf.properties[k] === 'string' &&
            ...
            if (authorCFs.length > bestAuthorCount) {
                ...
                const exampleTree = await fetch(siblingBase + '/' + sib + '/jcr:content.infinity.json', { headers: { 'CSRF-Token': token } }).then((r) => r.json());
