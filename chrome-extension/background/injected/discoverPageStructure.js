...
export async function discoverPageStructure(slug) {
    ...
    try {
        ...
        function walk(node, path, depth, parentType) {
            ...
            if (resType) {
                const entry = {
                    ...
                    properties: Object.keys(node).filter((k) => !k.startsWith('jcr:') && !k.startsWith('sling:') && !k.startsWith('cq:') && !k.startsWith(':') && typeof node[k] !== 'object'),
                };
        ...
        for (const c of containers) {
            const children = components.filter((comp) => comp.path.startsWith(c.path + '/') && comp.depth === c.depth + 1);
