...
export async function insertAuthorsInAEM(slug, authorPaths) {
  ...
  try {
    ...
    const existingCfContainers = Object.keys(gridNode).filter(
      (k) => k.startsWith('cf_container') && typeof gridNode[k] === 'object'
    )
    ...
    if (existingCfContainers.length > 0) {
      ...
      for (const name of existingCfContainers) {
        ...
        if (delRes.ok) log('  Eliminado: ' + name + ' ✓')
        else logWarn('  No se pudo eliminar ' + name + ': HTTP ' + delRes.status)
      }
    }
    ...
    for (let pairIdx = 0; pairIdx < pairs.length; pairIdx++) {
      ...
      for (let colIdx = 0; colIdx < 2; colIdx++) {
        if (colIdx >= pair.length) break // odd author count, skip node1
    ...
    try {
      ...
      if (verifyRes.ok) {
        ...
        const cfContainers = Object.keys(tree).filter(
          (k) => k.startsWith('cf_container') && typeof tree[k] === 'object'
        )
        ...
        for (const name of cfContainers) {
          ...
          for (const nodeName of ['node0', 'node1']) {
            ...
            if (!grid) continue
            const cfs = Object.keys(grid).filter(
              (k) => k.startsWith('contentfragment') && typeof grid[k] === 'object'
            )
