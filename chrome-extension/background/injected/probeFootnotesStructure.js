...
export async function probeFootnotesStructure(slug) {
    ...
    try {
        ...
        function childKeys(node) {
            return Object.keys(node).filter((k) => !k.startsWith('jcr:') && !k.startsWith('sling:') &&
        ...
        try {
            ...
            if (containers.length > 0) {
                ...
                for (let i = 0; i < containers.length; i++) {
                    ...
                    log('  [' + i + '] ' + shortPath);
                    if (props.numOfColumns)
                        log('      cols: ' + props.numOfColumns);
                }
            }
        }
        catch (e) {
        ...
        for (const sib of siblings) {
            if (results.siblingExamples.length >= 5)
        ...
        if (results.bestExample) {
            ...
            if (best.footnoteRTE.textAsJson) {
                ...
                try {
                    ...
                    if (prettyLines.length > 80)
                        log('    ... (' + prettyLines.length + ' lineas)');
                }
                catch (e) {
